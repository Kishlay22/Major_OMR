...
def displayImage(img,canvas,size=[]):   #Displays Image On A Specific Canvas
    ...
    canvas.original = img
    resize_img = cv2.resize(img,(int(size[0]),int(size[1])))
    canvas.sizeX = size[0]
    canvas.sizeY = size[1]
...
def deletetemp():
    global batches
    confirm = messagebox.askyesno("Confrim","If You Didn't Save Your Preset Then All of Your Settings will be lost. Are you sure you want to exit ?")
    if confirm:
...
def handleDetection(img,doCanvas,ce1Value=70,ce2Value=20,blurValue=17):
    ...
    try:
        img_detected = aos.findCorners(img,cp1=int(ce1Value),cp2=int(ce2Value),bp=blurValue)
...
def ImgZoomIn(canvas):
    try:
        newSize = [canvas.sizeX*zoomFactor,canvas.sizeY*zoomFactor]
...
def savePresetToFile(presetNameEntry,presetDescriptionEntry,savePresetWindow):
    ...
    if(name.find('$') >= 0) or (description.find('$') >= 0):
        messagebox.showerror("No Symbols","Please do not use '$' symbols in Preset Name or Description.")
        savePresetWindow.destroy()
    elif (len(batches) <= 0):
        messagebox.showerror("No Batches","Preset must have atleast 1 batch.")
        savePresetWindow.destroy()
    elif(len(name) <= 0):
        messagebox.showerror("No Name","Preset must have a name.")
        savePresetWindow.destroy()
    else:
        buffer = ""
        ...
        try:
            ...
            with open(".presets","a+") as f:
                f.write(buffer + "\n")
            savePresetWindow.destroy()
            messagebox.showinfo("Success","Preset Saved Successfuly And Ready to use")
        except Exception as e:
            savePresetWindow.destroy()
            messagebox.showerror("Error","Invalid datatype entered in some input box. Please check input fields and only enter integer value")
            print(e)
